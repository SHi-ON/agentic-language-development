...
describe('E03 primary and reserve seed manifest', () => {
  ...
  it('allocates disjoint prospective v2/v3 pilot namespaces without changing v1', () => {
    const original = buildE03SeedManifest('blinded-pilot', 20);
    const amended = buildE03SeedManifest('blinded-pilot', 20, 'v2');
    const third = buildE03SeedManifest('blinded-pilot', 20, 'v3');
    expect(amended.attemptVersion).toBe('v2');
    expect(third.attemptVersion).toBe('v3');
    ...
    expect(new Set([...allSeeds(original), ...allSeeds(amended), ...allSeeds(third)]).size).toBe(
      allSeeds(original).length + allSeeds(amended).length + allSeeds(third).length);
  });
