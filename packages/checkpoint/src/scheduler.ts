 ...
 *    alongside its `uncaughtException` / `unhandledRejection` handlers.
 */
...
export interface CheckpointSchedulerOptions {
  ...
  clock: Clock;
  /** How often {@link CheckpointScheduler.tick} runs; defaults to `timeIntervalMs`. */
...
export class CheckpointScheduler {
  ...
  constructor(options: CheckpointSchedulerOptions) {
    ...
    this.tickIntervalMs = options.tickIntervalMs ?? options.timeIntervalMs;
  ...
  private trigger(reason: CheckpointReason): void {
    ...
    this.chain = this.chain.then(async () => {
      try {
        const result = await this.service.createCheckpointIfChanged(
          this.runId,
          reason,
        );
        this.lastCheckpointAtMs = this.nowMs();
