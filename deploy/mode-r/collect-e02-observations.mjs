...
export async function collectE02Observations({ directory, seed, mode, profile, softwareCommit, registration, slot }) {
  ...
  if (registered) {
    ...
    assert.match(directory, /^\/evidence\/registered-e02-[1-5]$/u);
    assert.equal(directory, '/evidence/registered-e02-' + slot);
  } else {
  ...
  const runId = registered ? 'registered-e02-' + slot : 'e02-development';
