/**
 * `baby-b` twin pack (SPECIFICATION.md §12.4; BACKLOG ALD-048).
 *
 * Mirrors `baby-a`'s pack exactly, with the fixed role flipped. Every route
 * here is reachable only by `internal-gateway` or `internal-controller`
 * except `/ledger`, which is `researcher-viewer`'s audit-layer read (§12.2:
 * "never cross-exposed to a Baby"; this file reads only its own side).
 * There is no route, and no code path, that lets a request address
 * `baby-a`'s adapter (§4.2 absence-of-route): every body is checked with
 * `namesOtherBaby` before this pack's own adapter is touched.
 *
 * This pack does not import anything from `baby-a`.
 */
import {
...
const OWN_ROLE = 'baby-b' as const;
...
export const BABY_B_ROUTE_PATTERNS = [
  '/observe',
  '/act',
  '/deliver',
  '/outcome',
...
const routes: RouteDefinition[] = [
  ...
  },
  {
    ...
    handler: ({ request, actorId }) => {
      ...
      const runtime = getNurseryRuntime();
      const { babyB } = runtime.ledgers(runId);
      const { babyB: humanAudit } = runtime.auditLedgers(runId);
      const agentNativeEventCount = babyB.filter(
...
export default class BabyBPack implements BehaviorPack {
